...
export default function PfCashFlowPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [s, t, d, i] = await Promise.all([
        getPfAnalyticsSummary('cash-flow', { month: monthStr }),
        getPfAnalyticsTrend('cash-flow', { month: monthStr, type: view, year }),
        getPfAnalyticsDistribution('cash-flow', { month: monthStr }),
        getPfAnalyticsInsights('cash-flow', { month: monthStr }),
      ])
      setSummary(s || null)
      setTrend(t || null)
  ...
  const rows = series
  const accountFlow = summary?.account_flow || []
  const mom = summary?.comparison?.month_over_month_pct
  const noData = !loading && !err && !rows.length
  ...
  const accountBarData = useMemo(
    () =>
      accountFlow.slice(0, 10).map((x) => ({
        name: String(x.name || 'Account').slice(0, 28),
        net: Number(x.net) || 0,
      })),
    [accountFlow],
  )
  ...
  return (
    <div className="mx-auto max-w-[1440px] space-y-5">
      <header className="sticky top-0 z-20 rounded-xl border border-[var(--pf-border)] bg-[var(--pf-header)]/75 px-4 py-3 backdrop-blur-xl">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="text-sm text-[var(--pf-text-muted)]">Global money movement across income, expenses, ledger, loans, investments, and chit funds.</p>
          </div>
      ...
      <section className="grid grid-cols-2 gap-3 lg:grid-cols-5">
        ...
        <div className={cardCls}><p className="text-[11px] font-semibold uppercase text-[var(--pf-text-muted)]">Avg daily</p><p className={`mt-1 text-2xl font-bold ${avg >= 0 ? 'text-sky-400' : 'text-amber-300'}`}>{formatInr(avg)}</p></div>
      ...
      {loading ? (
        ...
        <>
          ...
          <section className={cardCls}>
            <div className="mb-2 flex items-center gap-2"><ChartBarIcon className="h-4 w-4 text-[var(--pf-primary)]" /><h2 className="text-[13px] font-semibold text-[var(--pf-text)]">Account flow</h2></div>
            <div style={{ height: 280 }}>
              {!accountBarData.length ? <div className="flex h-full items-center justify-center text-sm text-[var(--pf-text-muted)]">No account flow in this period</div> : (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={accountBarData} layout="vertical" margin={{ left: 8, right: 16 }}>
                    ...
                    <Bar dataKey="net" radius={[0, 4, 4, 0]}>
                      {accountBarData.map((x, idx) => <Cell key={idx} fill={(Number(x.net) || 0) >= 0 ? '#22c55e' : '#ef4444'} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              )}
          ...
          <section className={cardCls}>
            ...
            <div className={pfTableWrap}>
              <table className={pfTable}>
                ...
                <tbody>
                  {!rows.length ? (
                    <tr className={pfTrHover}><td colSpan={4} className={`${pfTd} text-[var(--pf-text-muted)]`}>No rows for this period</td></tr>
                  ) : (
                    rows.map((r, i) => (
